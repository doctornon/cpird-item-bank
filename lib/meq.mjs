...
export function validateMeq(row) {
 ...
 const bad=badImages(s.images) || s.questions.map(q=>badImages(q.images)).find(Boolean);
 if(bad) return `รูปภาพในตอนที่ ${i+1}: ${bad}`;
 }
 return '';
}
// รูปเก็บเป็นข้อมูลมีโครงสร้าง ไม่ใช่ HTML จึงไม่เปิดช่องให้ฝัง markup ลงในโจทย์
export function badImages(images) {
 if(images==null) return '';
 if(!Array.isArray(images)) return 'รูปแบบข้อมูลรูปไม่ถูกต้อง';
 for(const im of images) {
  if(!im || typeof im.url!=='string' || !/^https:\/\//.test(im.url)) return 'ที่อยู่รูปต้องเป็น https';
  if(im.width!=null && (!Number.isFinite(Number(im.width)) || Number(im.width)<10 || Number(im.width)>100)) return 'ความกว้างรูปต้องอยู่ระหว่าง 10-100%';
  if(im.align!=null && !['left','center','right'].includes(im.align)) return 'ตำแหน่งรูปไม่ถูกต้อง';
 }
 return '';
}
export const newQuestion=()=>({id:crypto.randomUUID(),prompt:'',points:0,modelAnswer:'',rubric:'',images:[]});
export const newStage=()=>({id:crypto.randomUUID(),title:'',scenario:'',minutes:5,images:[],questions:[newQuestion()]});
