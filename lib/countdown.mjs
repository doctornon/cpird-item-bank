...
// เตือนเรื่องเดดไลน์ล่วงหน้าเท่านี้ ก่อนหน้านั้นยังไม่ต้องเร่งผู้สมัคร
export const DEADLINE_WARN_DAYS = 7;

// แปลงแถวจากตาราง exam_schedule ให้เป็นรูปแบบที่ส่วนแสดงผลใช้
export const fromRow = (r) => ({
  ...
  years: r.years || [],
  where: r.where_text,
  closesAt: r.register_closes_at,
});

// สถานะการรับสมัครของรอบหนึ่ง เทียบกับเวลาปัจจุบันและสถานะของผู้ใช้
// เซิร์ฟเวอร์เป็นผู้บังคับเดดไลน์จริง ค่าที่ได้จากที่นี่ใช้เพื่อแสดงผลเท่านั้น
export function registrationState(exam, now = Date.now(), registered = false) {
  const closes = exam?.closesAt ? new Date(exam.closesAt).getTime() : null;
  if (closes == null || Number.isNaN(closes)) {
    return { closed: false, registered, left: null, warn: false, canRegister: !registered, canCancel: registered };
  }
  const left = timeLeft(exam.closesAt, now);
  const closed = now >= closes;
  return {
    closed,
    registered,
    left,
    // เตือนเมื่อใกล้ปิดรับและยังไม่ได้สมัคร
    warn: !closed && !registered && left.days < DEADLINE_WARN_DAYS,
    canRegister: !closed && !registered,
    canCancel: !closed && registered,
  };
}

export function deadlineLabel(state) {
  if (!state) return "";
  if (state.closed) return state.registered ? "ปิดรับสมัครแล้ว · คุณสมัครไว้แล้ว" : "ปิดรับสมัครแล้ว";
  const l = state.left;
  if (!l) return "";
  if (l.days > 0) return `ปิดรับสมัครในอีก ${l.days} วัน`;
  if (l.hours > 0) return `ปิดรับสมัครในอีก ${l.hours} ชั่วโมง`;
  return `ปิดรับสมัครในอีก ${l.minutes} นาที`;
}
