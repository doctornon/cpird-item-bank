...
export default function DeliveryManager({sb}){
 ...
 const [ftab,setFtab]=useState('exam'),[saveDefault,setSaveDefault]=useState(false),[templates,setTemplates]=useState([]),[tplId,setTplId]=useState(''),[tplName,setTplName]=useState('');
 const applyTemplate=(id)=>{setTplId(id);const t=templates.find(x=>x.id===id);if(!t)return;setForm(f=>{const n={...f};for(const k of DEFAULT_KEYS)if(k in (t.settings||{}))n[k]=t.settings[k];return n;});setTplName(t.name);try{localStorage.setItem('cpird_round_tpl',id);}catch{}};
 const deleteTemplate=()=>run(async()=>{const t=templates.find(x=>x.id===tplId);if(!t||!window.confirm(`ลบเทมเพลต "${t.name}"?`))return;await rpc('template_delete',{id:tplId});setTplId('');setTplName('');try{localStorage.removeItem('cpird_round_tpl');}catch{}setTemplates(await rpc('template_list'));setMessage('ลบเทมเพลตแล้ว');});
 ...
 const load=useCallback(async()=>{setLoading(true);try{
  const [list,mcq,meq,c,tpl]=await Promise.all([rpc('list'),sb.from('exam_sets').select('id,name,active_state').eq('kind','mcq').in('active_state',['active','pending']).order('active_state').order('updated_at',{ascending:false}),sb.from('exam_sets').select('id,name,active_state').eq('kind','meq').in('active_state',['active','pending']).order('active_state').order('updated_at',{ascending:false}),sb.from('medical_centers').select('id,name_th,short_name').eq('is_active',true).order('display_order'),rpc('template_list').catch(()=>[])]);
  for(const r of [mcq,meq,c])if(r.error)throw Error(r.error.message);
  setRows(list);setSources({mcq:mcq.data||[],meq:(meq.data||[]).map(s=>({...s,id:`set:${s.id}`}))});setCenters((c.data||[]).map(x=>({id:x.id,name:x.short_name||x.name_th})));setTemplates(Array.isArray(tpl)?tpl:[]);
 ...
 const startCreate=()=>{setForm(blank());setFtab('exam');setSelected(null);setGrading(null);setError('');setSaveDefault(false);let last='';try{last=localStorage.getItem('cpird_round_tpl')||'';}catch{}if(last&&templates.some(t=>t.id===last))applyTemplate(last);else{setTplId('');setTplName('');}};
 const create=event=>{event.preventDefault();run(async()=>{
  ...
  let savedTpl=false;
  if(saveDefault&&tplName.trim()){const d={};for(const k of DEFAULT_KEYS)d[k]=form[k];const res=await rpc('template_save',{name:tplName.trim(),settings:d});try{if(res?.id)localStorage.setItem('cpird_round_tpl',res.id);}catch{}savedTpl=true;}
  setForm(null);setSelected(data);setRoster([]);await load();setMessage('บันทึกร่างแล้ว ยังไม่เปิดให้ผู้สอบเข้าทำ'+(savedTpl?` · บันทึกเทมเพลต "${tplName.trim()}" แล้ว (จะถูกใช้เป็นค่าเริ่มต้นครั้งถัดไป)`:''));
 });};
 ...
 {form?<form className="delivery-paper delivery-form" onSubmit={create}><h2>สร้างรอบสอบใหม่</h2>
  ...
  <fieldset disabled={busy} style={{border:0,padding:0,margin:0}} onChangeCapture={()=>{}}>
   <div hidden={ftab!=='exam'}>
    <div className="row" style={{gap:8,alignItems:'end',flexWrap:'wrap',marginBottom:10}}>
     <label style={{flex:1,minWidth:240}}>ใช้เทมเพลตการตั้งค่า<select value={tplId} onChange={e=>applyTemplate(e.target.value)}><option value="">— ไม่ใช้เทมเพลต (ค่ามาตรฐาน) —</option>{templates.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label>
     {tplId&&<button type="button" className="btn ghost sm" disabled={busy} onClick={deleteTemplate}>ลบเทมเพลตนี้</button>}
    </div>
    {tplId&&<p className="muted" style={{marginTop:-4}}>เติมค่าจากเทมเพลตแล้ว (แก้ต่อได้ · ชื่อ/ชุด/เวลาเปิด-ปิด ต้องกรอกเอง) — เทมเพลตล่าสุดที่ใช้จะถูกเลือกอัตโนมัติครั้งถัดไป</p>}
    <label>ชื่อรอบสอบ<input maxLength={300} value={form.title} onChange={e=>set('title',e.target.value)}/></label>
    <label className="delivery-check"><input type="checkbox" checked={!!form.is_demo} onChange={e=>set('is_demo',e.target.checked)}/>🧪 ตัวอย่างห้องทดสอบ (demo) — ให้ทดลองใช้งานก่อนวันจริง · ไม่นับผล/สถิติ · ใช้ชุดที่มีข้อสอบตัวอย่างได้</label>
  ...
  <div className="delivery-actions" style={{alignItems:'center'}}><div className="row" style={{gap:8,alignItems:'center',marginRight:'auto',flexWrap:'wrap'}}><label className="delivery-check" style={{margin:0}}><input type="checkbox" checked={saveDefault} onChange={e=>{setSaveDefault(e.target.checked);if(e.target.checked&&!tplName)setTplName(form.title||'');}}/>บันทึกการตั้งค่านี้เป็นเทมเพลต</label>{saveDefault&&<input style={{minWidth:220}} placeholder="ชื่อเทมเพลต เช่น NLE มาตรฐาน" maxLength={120} value={tplName} onChange={e=>setTplName(e.target.value)}/>}</div><button type="button" className="btn ghost" disabled={busy} onClick={()=>setForm(null)}>ยกเลิก</button><button className="btn" disabled={busy}>บันทึกร่าง</button></div></form>:
