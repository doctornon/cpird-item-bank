...
export default function DeliveryPortal({sb,profile,onExit,onSignOut}) {
 ...
 const [blurN,setBlurN]=useState(0);
 ...
 useEffect(()=>{
  ...
  const onHide=()=>{if(document.visibilityState==='hidden'){setBlurN(n=>n+1);const a=current.current;if(a)sb.rpc('delivery_run',{action:'blur',payload:{attempt_id:a.id}}).catch(()=>{});}};
