...
export default function DeliveryPortal({sb,profile,onExit,onSignOut}) {
 ...
 const header=<header className="delivery-heading"><div><h1>ห้องสอบของฉัน</h1><p>ระบบจัดทดสอบและวัดผล สพพ. · {profile?.full_name||profile?.email}</p></div><div className="row">{onExit&&<button className="btn ghost" onClick={onExit}>กลับหน้าจัดการ</button>}{onSignOut&&<button className="btn ghost" onClick={onSignOut}>ออกจากระบบ</button>}</div></header>;
