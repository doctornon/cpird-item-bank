...
export default function DeliveryPortal({sb,profile,onExit,onSignOut,embedded}) {
 ...
 const [reading,setReading]=useState(null),[layout,setLayout]=useState('split');
 useEffect(()=>{try{const v=localStorage.getItem('cpird_meq_layout');if(v==='split'||v==='stack')setLayout(v);}catch{}},[]);
 const setLayoutPersist=v=>{setLayout(v);try{localStorage.setItem('cpird_meq_layout',v);}catch{}};
 ...
 const openReading=async(a)=>{setBusy(true);setError('');try{const {data,error}=await sb.rpc('meq_practice_read',{_exam_id:a.id});if(error)throw Error(error.message);setReading(data);}catch(e){setError(e.message||'อ่านทบทวนไม่สำเร็จ');}finally{setBusy(false);}};
 ...
 const rootClass=embedded?'delivery':'wrap section delivery';
 const header=<header className="delivery-heading"><div><h1>ห้องสอบของฉัน</h1><p>ระบบจัดทดสอบและวัดผล สพพ. · {profile?.full_name||profile?.email}</p></div>{!embedded&&<div className="row">{onExit&&<button className="btn ghost" onClick={onExit}>กลับหน้าจัดการ</button>}{onSignOut&&<button className="btn ghost" onClick={onSignOut}>ออกจากระบบ</button>}</div>}</header>;
 if(!attempt)return <Root className={rootClass}>{header}{alert}{reading?<section className="delivery-paper meq-read"><button className="btn ghost" onClick={()=>setReading(null)}>‹ กลับรายการสอบ</button><h2>{reading.title}</h2><p className="delivery-alert" role="note">📖 โหมดอ่านทบทวน — ไม่จับเวลา ไม่นับคะแนน ดูโจทย์และคำถามได้ทั้งชุด</p>{(reading.paper||[]).map((s,si)=><div key={si} className="meq-read-stage"><h3>{s.title||('ตอนที่ '+(si+1))}</h3><p className="delivery-text" style={{whiteSpace:'pre-wrap'}}>{s.scenario}</p>{(s.questions||[]).map((q,qi)=><div key={qi} className="meq-read-q"><p className="delivery-stem" style={{whiteSpace:'pre-wrap'}}><b>ข้อ {qi+1}</b>{q.points!=null?` (${q.points} คะแนน)`:''} — {q.prompt||q.stem}</p>{q.modelAnswer&&<p className="delivery-text"><b>แนวคำตอบ:</b> {q.modelAnswer}</p>}{q.rubric&&<p className="delivery-text"><b>เกณฑ์ให้คะแนน:</b> {q.rubric}</p>}</div>)}</div>)}<button className="btn ghost" onClick={()=>setReading(null)}>‹ กลับรายการสอบ</button></section>:intro?<section className="delivery-paper"><button className="btn ghost" onClick={()=>setIntro(null)}>กลับรายการสอบ</button>{intro.is_demo&&demoBanner}<h2>{intro.title}</h2><p>{intro.kind.toUpperCase()} · {intro.count} {intro.kind==='meq'?'ตอน':'ข้อ'} · {intro.duration_min} นาที · ทำได้ {intro.attempts_allowed} ครั้ง</p><h3>คำชี้แจงก่อนเริ่ม</h3><p className="delivery-text">{intro.instructions||'อ่านโจทย์และเลือกหรือพิมพ์คำตอบให้ครบก่อนส่งข้อสอบ'}</p><ul><li>เวลาเริ่มนับเมื่อกดเริ่มสอบ และยังเดินต่อแม้ปิดหน้าเว็บ</li><li>{intro.kind==='meq'?'เปิดทีละตอน หมดเวลาจะล็อกคำตอบและเปิดตอนถัดไป ห้ามอ่านล่วงหน้าหรือย้อนแก้ไข':intro.allow_back?'ย้อนกลับแก้คำตอบและปักหมุดเพื่อทบทวนได้':'ทำทีละข้อ เมื่อยืนยันไปข้อถัดไปแล้วจะย้อนแก้ไม่ได้'}</li><li>ระบบบันทึกคำตอบทุก 3 วินาที ตรวจสถานะบันทึกก่อนปิดหน้าต่าง</li><li>{intro.show_score?'แสดงคะแนนหลังส่ง (MEQ รออาจารย์ตรวจ)':'รอบนี้ไม่แสดงคะแนนให้ผู้สอบ'} · {intro.show_answers?'เปิดเฉลยหลังส่งและตรวจเสร็จ':'ไม่เปิดเฉลย'}</li>{intro.proctor_blur&&<li>รอบสอบนี้เฝ้าระวังการสลับแท็บ/ออกจากหน้าจอ และบันทึกจำนวนครั้ง</li>}<li>ปิดรอบสอบ: {date(intro.close_at)} — จะสิ้นสุดไม่เกินเวลาปิดนี้</li></ul>{intro.access_required&&<label>รหัสเข้าสอบ<input value={code} onChange={e=>setCode(e.target.value)} autoComplete="off" /></label>}<label className="delivery-check"><input type="checkbox" checked={agreed} onChange={e=>setAgreed(e.target.checked)}/>อ่านคำชี้แจงและพร้อมเริ่มสอบ</label><button className="btn" disabled={!agreed||busy} onClick={()=>start(intro)}>{busy?'กำลังเปิดข้อสอบ…':'เริ่มสอบและจับเวลา'}</button></section>:<><ExamCountdown sb={sb} studentYear={profile?.year_level} notify={setError}/>{loading?<p role="status">กำลังโหลดรายการสอบ…</p>:!list.length?<section className="delivery-paper"><h2>{error?'โหลดรายการไม่สำเร็จ':'ยังไม่มีรอบสอบที่เปิดให้คุณ'}</h2><p>เมื่ออาจารย์จัดชุดข้อสอบและเปิดรอบสอบแล้ว รายการจะปรากฏที่นี่</p><button className="btn ghost" onClick={load}>โหลดรายการอีกครั้ง</button></section>:<div className="exam-cards">{list.map(a=>{
   const active=a.attempts?.find(t=>t.status==='in_progress');
   const done=(a.attempts||[]).filter(t=>t.status!=='in_progress');
   const now=Date.now();
   const notYet=a.open_at&&now<new Date(a.open_at).getTime();
   const closed=a.status!=='open'||(a.close_at&&now>=new Date(a.close_at).getTime());
   const usedUp=(a.attempts?.length||0)>=a.attempts_allowed;
   const available=!closed&&!notYet&&!usedUp;
   const state=active?'active':available?'available':(usedUp&&done.length)?'done':notYet?'soon':'closed';
   const badge={active:['กำลังทำอยู่','mcq'],available:['เปิดให้สอบ','approved'],done:['ทำครบแล้ว','retired'],closed:['ปิดแล้ว','retired'],soon:['ยังไม่เปิด','draft']}[state];
   const inactive=state==='done'||state==='closed'||state==='soon';
   return <article key={a.id} className={"exam-card"+(inactive?" exam-card-off":"")+(a.is_demo?" is-demo":"")}>
    <div className="exam-card-top"><span className={"pill "+a.kind}>{a.kind.toUpperCase()}</span><span className={"pill "+badge[1]}>{badge[0]}</span>{a.is_demo&&<span className="pill draft">🧪 ตัวอย่าง · ไม่นับคะแนน</span>}</div>
    <h2>{a.title}</h2>
    <div className="exam-card-meta"><span>⏱ {a.duration_min>=100000?'ไม่จำกัดเวลา':a.duration_min+' นาที'}</span><span>📝 {a.count} {a.kind==='meq'?'ตอน':'ข้อ'}</span><span>🔁 {a.attempts_allowed>=9999?'ไม่จำกัดครั้ง':'ทำได้ '+a.attempts_allowed+' ครั้ง'}{done.length?` · ทำไปแล้ว ${done.length}`:''}</span></div>
    <div className="exam-card-dates"><span>เปิด {date(a.open_at)}</span><span>ปิด {date(a.close_at)}</span></div>
    {done.length>0&&<div className="exam-card-results">{done.map(t=><button className="btn ghost sm" key={t.id} onClick={()=>openResult(t.id)} disabled={busy}>ผลครั้งที่ {t.attempt_no}{t.status==='awaiting_grade'?' · รอตรวจ':''}</button>)}</div>}
    <div className="exam-card-action" style={{gap:8,flexWrap:'wrap'}}><button className="btn" disabled={busy||(!active&&!available)} onClick={()=>{if(active)openResult(active.id);else{setIntro(a);setCode('');setAgreed(false);setError('');}}}>{active?'ทำต่อ':available?(done.length?'สอบอีกครั้ง':a.is_demo?'🧪 เริ่มฝึกทำ (จับเวลา)':'อ่านคำชี้แจง & เริ่มสอบ'):badge[0]}</button>{a.is_demo&&a.kind==='meq'&&<button className="btn ghost" disabled={busy} onClick={()=>openReading(a)}>📖 อ่านทบทวน</button>}</div>
   </article>;})}</div>}</>}</Root>;
 if(attempt.status!=='in_progress'){
  ...
  if(showCert&&passedCert)return <Root className={rootClass}><div className="row sa-noprint" style={{gap:8,marginBottom:16}}><button className="btn ghost" onClick={()=>setShowCert(false)}>‹ กลับ</button><button className="btn" onClick={()=>window.print()}>🖨 พิมพ์ใบประกาศ</button></div><section className="certificate"><div className="cert-brand">คลังข้อสอบ CPIRD · MEC</div><h1>ใบประกาศผลการสอบ</h1><p className="cert-sub">ขอรับรองว่า</p><h2>{profile?.full_name||profile?.email}</h2><p className="cert-sub">ได้ผ่านการสอบ</p><h3>{attempt.title}</h3><p className="cert-score">คะแนน {attempt.score} / {attempt.max_score} ({percent}%) · เกณฑ์ผ่าน {attempt.pass_mark}%</p><p className="cert-date">วันที่ {date(attempt.submitted_at)}</p></section></Root>;
  return <Root className={rootClass}>{header}{alert}{attempt.is_demo&&demoBanner}<section className="delivery-paper"><h2>ส่งข้อสอบแล้ว</h2><p>{attempt.title} · ส่งเมื่อ {date(attempt.submitted_at)}</p>{attempt.status==='awaiting_grade'?<p className="delivery-result">รออาจารย์ตรวจ MEQ</p>:percent!=null?<p className="delivery-result">{attempt.score} / {attempt.max_score} คะแนน ({percent}%) · {percent>=attempt.pass_mark?'ผ่าน':'ไม่ผ่าน'}</p>:<p>รอบสอบนี้ไม่แสดงคะแนน</p>}<div className="row" style={{gap:8}}><button className="btn ghost" onClick={home}>กลับรายการสอบ</button>{passedCert&&<button className="btn" onClick={()=>setShowCert(true)}>🎓 ใบประกาศ</button>}</div></section>{!qs.length?<p>{attempt.show_answers&&attempt.feedback_at&&!attempt.answers_open?`เฉลย/feedback จะเปิดให้ดูได้ตั้งแต่ ${date(attempt.feedback_at)}`:'ยังไม่เปิดเฉลยสำหรับรอบสอบนี้'}</p>:qs.map((q,i)=><section className="delivery-paper" key={q.id}><h3>ข้อ {i+1}</h3><p className="delivery-text">{q.stem||q.prompt}</p><StemImages images={q.images}/><p>คำตอบของคุณ: {attempt.answers[q.id]||'ไม่ได้ตอบ'}</p>{q.options?.map(o=><div key={o.id}><p style={{margin:0}}>{o.id}. {o.body}{o.correct?' · คำตอบที่ถูก':''}{o.rationale?` — ${o.rationale}`:''}</p><OptImage url={o.image_url} width={o.image_width}/></div>)}<p className="delivery-text">{q.rationale||q.modelAnswer}</p>{q.rubric&&<p className="delivery-text">เกณฑ์ให้คะแนน: {q.rubric}</p>}{attempt.grades[q.id]&&<p>คะแนน {attempt.grades[q.id].points} / {q.points} · {attempt.grades[q.id].comment||''}</p>}</section>)}</Root>;
 }
 ...
 return <Root className={'delivery delivery-active'+(embedded?' delivery-embed-fs':'')+(attempt.block_copy?' no-copy':'')} {...guard}><header className="delivery-exam-bar"><div><strong>{attempt.title}</strong><span>{meq?`MEQ · ตอนที่ ${attempt.stage+1}/${attempt.count} · ${qs.length} ข้อในตอนนี้`:`MCQ · ${attempt.count} ข้อ`}</span></div><div className={'delivery-clock'+(seconds<=60?' urgent':'')} role="timer" aria-label="เวลาที่เหลือ">{clock(seconds)}</div>{!meq&&<button className="btn" onClick={()=>setReview(true)} disabled={locked}>ตรวจทานและส่ง</button>}</header><div className="wrap section">{attempt.is_demo&&demoBanner}{alert}{attempt.proctor_blur&&blurN>0&&<div className="delivery-alert" role="alert">⚠ ตรวจพบการสลับแท็บ/ออกจากหน้าจอ {blurN} ครั้ง (รวม {awayS} วินาที) — ระบบบันทึกไว้แล้ว กรุณาอยู่ในหน้าสอบจนกว่าจะส่ง</div>}<div className="delivery-sync" role="status"><span>{pending?`รอบันทึก ${pending} คำตอบ`:sync||'คำตอบล่าสุดจากเซิร์ฟเวอร์'}</span>{meq&&<button type="button" className="btn ghost sm" onClick={()=>setLayoutPersist(layout==='split'?'stack':'split')} title="สลับมุมมองการอ่านโจทย์และช่องพิมพ์ตอบ">{layout==='split'?'⬍ มุมมองบน-ล่าง':'⬌ มุมมองสองคอลัมน์'}</button>}<button className="btn ghost sm" disabled={busy} onClick={()=>transact(seconds<=0?'paper':'save')}>บันทึก / เชื่อมต่ออีกครั้ง</button></div>{review?<section className="delivery-paper"><h2>{review==='advance'?'ยืนยันล็อกคำตอบและไปต่อ':sequential?'ยืนยันคำตอบ':'ตรวจทานก่อนส่ง'}</h2><p>ทำแล้ว {answered} / {qs.length} {meq?'คำถามในตอนนี้':'ข้อ'} · ยังไม่ทำ {qs.length-answered}{skipped?` (ข้ามไว้ ${skipped})`:''}{unsureN?` · ไม่แน่ใจ ${unsureN}`:''}</p>{meq&&review!=='advance'&&<p>หากส่งข้อสอบทั้งหมด ตอนที่ยังไม่เปิดจะไม่ได้รับคำตอบ</p>}<p>{review==='advance'?'เมื่อไปต่อแล้วจะย้อนกลับแก้คำตอบนี้ไม่ได้':'เมื่อยืนยันส่งแล้วจะไม่สามารถแก้คำตอบได้'}</p>{mustAll&&review!=='advance'&&answered<qs.length&&<p className="delivery-alert" role="alert">รอบสอบนี้บังคับตอบให้ครบทุกข้อก่อนส่ง — ยังเหลือ {qs.length-answered} ข้อ</p>}<div className="row"><button className="btn ghost" onClick={()=>setReview(false)} disabled={busy}>กลับไปทำต่อ</button><button className="btn" disabled={locked||(mustAll&&review!=='advance'&&answered<qs.length)} onClick={()=>transact(review==='advance'?'advance':'submit')}>{review==='advance'?'ยืนยันล็อกคำตอบและไปต่อ':'ยืนยันส่งข้อสอบทั้งหมด'}</button></div></section>:<div className="delivery-layout"><aside className="delivery-nav"><h2>{meq?`ตอนที่ ${attempt.stage+1} · ${qs.length} ข้อ`:'รายการข้อสอบ'}</h2><p>ทำแล้ว <b style={{color:'var(--good)'}}>{answered}</b> · เหลือ <b>{qs.length-answered}</b> จาก {qs.length} ข้อ</p>{!sequential&&<div className="qfilter" role="tablist">{[['all','ทุกข้อ'],['unanswered','ยังไม่ทำ'],['unsure','ไม่แน่ใจ'],['skipped','ข้าม']].map(([v,l])=><button key={v} type="button" className={'status-filter'+(filter===v?' selected':'')} aria-pressed={filter===v} onClick={()=>setFilter(v)}>{l}</button>)}</div>}<nav className="qgrid" aria-label="เลือกข้อสอบ">{qs.map((item,i)=>{const done=!!String(answers[item.id]||'').trim();const skip=visited[item.id]&&!done;const unsure=!!flags[item.id];if(!(filter==='all'||(filter==='unanswered'&&!done)||(filter==='skipped'&&skip)||(filter==='unsure'&&unsure)))return null;return <button key={item.id} className={'qbtn'+(index===i?' current':unsure?' unsure':done?' answered':skip?' skipped':'')} aria-current={index===i?'step':undefined} aria-label={`ข้อ ${i+1}${done?' ทำแล้ว':skip?' ข้าม ยังไม่ทำ':' ยังไม่ทำ'}${unsure?' ไม่แน่ใจ':''}`} onClick={()=>{go(i);if(allMode)document.getElementById('q-'+item.id)?.scrollIntoView({behavior:'smooth',block:'start'});}}>{meq?i+1:(item.position??i)+1}</button>;})}</nav><div className="qlegend"><span><i className="cur"/>ข้อปัจจุบัน</span><span><i className="done"/>ทำแล้ว</span>{!sequential&&<span><i className="unsure"/>ไม่แน่ใจ</span>}{!sequential&&<span><i className="skip"/>ข้าม (ดูแล้วยังไม่ทำ)</span>}<span><i className="todo"/>ยังไม่ทำ</span></div>{sequential&&<p>ตอนหรือข้อก่อนหน้าถูกล็อกแล้ว</p>}</aside>{allMode?<section className="delivery-paper">{qs.map((item,i)=><div key={item.id} id={'q-'+item.id} className="delivery-q-all"><div className="delivery-question-head"><h2>ข้อ {(item.position??i)+1}</h2><button className={'btn ghost sm'+(flags[item.id]?' unsure-on':'')} aria-pressed={!!flags[item.id]} disabled={locked} onClick={()=>toggleFlag(item.id)}>{flags[item.id]?'★ ทำเครื่องหมายไม่แน่ใจ':'ไม่แน่ใจ?'}</button></div><p className="delivery-stem">{item.stem}</p><StemImages images={item.images}/><fieldset className="delivery-options" disabled={locked}><legend className="sr-only">เลือกคำตอบหนึ่งข้อ</legend>{item.options?.map((o,j)=><label key={o.id} className={'delivery-option'+(answers[item.id]===o.id?' chosen':'')}><input type="radio" name={`answer-${item.id}`} checked={answers[item.id]===o.id} onChange={()=>answer(item.id,o.id)}/><span className="delivery-option-label">{String.fromCharCode(65+j)}</span><span>{o.body}<OptImage url={o.image_url} width={o.image_width}/></span></label>)}</fieldset></div>)}<div className="delivery-actions"><button className="btn" disabled={locked} onClick={()=>setReview(true)}>ตรวจทานและส่ง</button></div></section>:<section className={'delivery-paper'+(meq?' meq-work meq-'+layout:'')}>{meq&&<div className="delivery-scenario meq-scenario"><h2>{attempt.paper[0]?.title||`ตอนที่ ${attempt.stage+1}`} <span className="meq-qcount">· {qs.length} ข้อ</span></h2><p className="delivery-text">{attempt.paper[0]?.scenario}</p><StemImages images={attempt.paper[0]?.images}/></div>}<div className={meq?'meq-answer':undefined}>{q?<><div className="delivery-question-head"><h2>{meq?`ข้อ ${index+1} / ${qs.length} · ${q.points} คะแนน`:`ข้อ ${(q.position??index)+1}`}</h2>{!meq&&<button className={'btn ghost sm'+(flags[q.id]?' unsure-on':'')} aria-pressed={!!flags[q.id]} disabled={locked} onClick={()=>toggleFlag(q.id)}>{flags[q.id]?'★ ทำเครื่องหมายไม่แน่ใจ':'ไม่แน่ใจ?'}</button>}</div><p className="delivery-stem">{q.stem||q.prompt}</p><StemImages images={q.images}/>{meq?<label className="meq-answer-field">คำตอบของคุณ<textarea maxLength={30000} rows={layout==='split'?18:10} disabled={locked} value={answers[q.id]||''} onChange={e=>answer(q.id,e.target.value)} placeholder="พิมพ์คำตอบที่นี่…"/></label>:<fieldset className="delivery-options" disabled={locked}><legend className="sr-only">เลือกคำตอบหนึ่งข้อ</legend>{q.options?.map((o,i)=><label key={o.id} className={'delivery-option'+(answers[q.id]===o.id?' chosen':'')}><input type="radio" name={`answer-${q.id}`} checked={answers[q.id]===o.id} onChange={()=>answer(q.id,o.id)}/><span className="delivery-option-label">{String.fromCharCode(65+i)}</span><span>{o.body}<OptImage url={o.image_url} width={o.image_width}/></span></label>)}</fieldset>}<div className="delivery-actions"><button className="btn ghost" disabled={index===0||locked} onClick={()=>go(index-1)}>ข้อก่อนหน้า</button>{index<qs.length-1?<button className="btn" disabled={locked} onClick={()=>go(index+1)}>ข้อถัดไป</button>:meq?<span className="delivery-wait">⏳ ครบทุกข้อในตอนนี้แล้ว — เมื่อหมดเวลา ระบบจะพาไปตอนถัดไปอัตโนมัติ</span>:sequential&&attempt.stage<attempt.count-1?<button className="btn" disabled={locked} onClick={()=>setReview('advance')}>ล็อกคำตอบและไปข้อถัดไป</button>:<button className="btn" disabled={locked} onClick={()=>setReview(true)}>ตรวจทานและส่ง</button>}</div></>:<p>กำลังโหลดข้อสอบ…</p>}</div></section>}</div>}</div></Root>;
}
