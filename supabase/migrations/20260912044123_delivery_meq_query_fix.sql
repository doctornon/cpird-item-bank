...
create or replace function exam_delivery.run(action text,payload jsonb) returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 ...
 if action='list' then
  perform exam_delivery.finish(a.id) from exam_delivery.attempts a where a.user_id=uid and a.status='in_progress' and now()>=a.expires_at;
 ...
 if action in ('save','advance','submit') and t.status='in_progress' then
  ...
  for kv in select * from jsonb_each_text(coalesce(payload->'answers','{}')) loop
   ...
   if e.kind='mcq' then
    ...
    if not exists(select 1 from jsonb_array_elements(t.paper->t.current_stage->'questions') xq where xq.value->>'id'=kv.key) then raise exception 'ตอนนี้ถูกล็อกหรือยังไม่เปิด'; end if;
   end if;
 ...
 if t.status='in_progress' then
  if e.kind='mcq' then
   ...
   select jsonb_agg(jsonb_build_object('id',x.q->>'id','prompt',x.q->>'prompt','points',x.q->'points') order by x.ord) into opts from jsonb_array_elements(s->'questions') with ordinality as x(q,ord);
